...
/**
    ...
    public function actionPrint() 
    {
        ...
        if($request = Yii::app()->request->getQuery('Check'))
        {
            $model->attributes=$request;
            $checkDate = date("d.m.Y", strtotime($request['date']));
            
            $printOutput = $this->renderPartial('_printDoc', array(
                'checks'=>$model), true);
            ...
            header("Content-Disposition: attachment; filename=\"Поверка $checkDate.doc");
            header("Content-Transfer-Encoding: binary");
            
            ob_clean();
            flush();
            // Преобразуем кодировку, чтобы кирилица нормально отображалась в MS Word
            $printOutput = iconv("utf-8", "windows-1251", $printOutput);
            echo $printOutput;
            Yii::app()->end();
        }
        else
            // Если параметров не было, редиректим на индекс
            $this->redirect(array('check/index'));
    }
}
